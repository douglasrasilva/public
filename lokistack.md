...
Cada recurso contém annotations utilizadas pelo ArgoCD, que definem a ordem de aplicação por meio do campo sync-wave.
Essas mesmas annotations podem ser usadas como referência para uma aplicação manual. Basta aplicar cada YAML somente após a criação bem-sucedida do recurso anterior, seguindo a sequência numérica definida no sync-wave.
